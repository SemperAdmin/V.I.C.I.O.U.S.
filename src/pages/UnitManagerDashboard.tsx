...
export default function UnitManagerDashboard() {
  ...
  const pendingInboundRows = useMemo(() => {
    ...
    return unitFormStatus.filter(row => {
      ...
      if (lowerCompany && !(row.company || '').toLowerCase().includes(lowerCompany)) return false
  ...
  }, [unitFormStatus, memberMap, filterMember, filterCompany, filterSection, filterForm])
