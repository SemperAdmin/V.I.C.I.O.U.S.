...
export default function CompanyManagerDashboard() {
  ...
  const [companyFormStatus, setCompanyFormStatus] = useState<Array<{ user_id: string; edipi?: string; form_name: string; kind: 'Inbound' | 'Outbound'; total: number; cleared: number; status: 'In_Progress' | 'Completed'; created_at?: string; completed_at?: string; arrival_date?: string; departure_date?: string; section?: string }>>([])
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      // Load submissions for the company
      if (import.meta.env.VITE_USE_SUPABASE === '1') {
        try {
          const unitAllSubs = await sbListSubmissionsByUnit(user.unit_id)

          // Filter submissions by company - members whose company_id matches
          const companySubmissions = unitAllSubs.filter(s => {
            const p = map[s.user_id]
            const memberCompany = p?.company_id || (s as any).member?.company_id || ''
            return memberCompany === myCompanyId
          })

          const inboundSubs = companySubmissions.filter(s => s.kind === 'Inbound')
          const outboundSubs = companySubmissions.filter(s => s.kind === 'Outbound')
          ...
          const rows: Array<{ user_id: string; edipi?: string; form_name: string; kind: 'Inbound' | 'Outbound'; total: number; cleared: number; status: 'In_Progress' | 'Completed'; created_at?: string; completed_at?: string; arrival_date?: string; departure_date?: string; section?: string }> = []
          for (const s of companySubmissions) {
  ...
  return (
    <div className="min-h-screen bg-github-dark">
      ...
      <main className="px-4 sm:px-6 lg:px-8 py-8 w-full">
        <div className="mb-4">
          <h1 className="text-white text-xl font-semibold">Company Manager - {companyLabel}</h1>
          <p className="text-gray-400 text-sm">Viewing all members in your company</p>
        </div>
          ...
          <div className="p-6">
            {tab === 'inbound' && (
              <div className="space-y-6">
                ...
                {inboundView === 'Pending' && (
                  <div className="space-y-6">
                    <div className="text-gray-300">Pending inbound processes in your company</div>
                ...
                {inboundView === 'Completed' && (
                  <div className="space-y-6">
                    <div className="text-gray-300">Completed inbound processes in your company</div>
                    <div className="overflow-x-auto">
                    <table className="min-w-full table-fixed text-xs sm:text-sm">
                      <thead className="text-gray-400">
                        <tr>
            ...
            {tab === 'outbound' && (
              <div className="space-y-6">
                ...
                {outboundView === 'Pending' && (
                  <div className="space-y-6">
                    <div className="text-gray-300">Pending outbound processes in your company</div>
                    <div className="overflow-x-auto">
                    <table className="min-w-full table-fixed text-xs sm:text-sm">
                      <thead className="text-gray-400">
                        <tr>
                ...
                {outboundView === 'Completed' && (
                  <div className="space-y-6">
                    <div className="text-gray-300">Completed outbound processes in your company</div>
                    <div className="overflow-x-auto">
                    <table className="min-w-full table-fixed text-xs sm:text-sm">
                      <thead className="text-gray-400">
                        <tr>
